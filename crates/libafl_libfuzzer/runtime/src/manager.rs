...
#[cfg(all(unix, not(miri)))]
use libafl::events::EVENTMGR_SIGHANDLER_STATE;
use libafl::events::{
    AwaitRestartSafe, EventWithStats, ExecStats, ProgressReporter, std_on_restart,
};
use libafl::{
    Error, HasMetadata,
    events::{
        BrokerEventResult, Event, EventFirer, EventManagerId, EventReceiver, EventRestarter,
        HasEventManagerId, SendExiting,
    },
...
use libafl::{monitors::stats::ClientStats, state::HasSolutions};
...
/// A simple, single-threaded event manager that handles events and works with
/// the [`LibFuzzerMonitor`] to ensure events are logged in the same way as
/// LibFuzzer would
pub struct LibFuzzerEventManager<I, MT, S> {
    /// The monitor
    monitor: MT,
    ...
    I: Debug,
{
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        f.debug_struct("LibFuzzerEventManager")
...
impl<I, MT, S> EventFirer<I, S> for LibFuzzerEventManager<I, MT, S>
where
    I: Debug,
    MT: Monitor,
    S: Stoppable,
...
impl<I, MT, S> SendExiting for LibFuzzerEventManager<I, MT, S> {
...
impl<I, MT, S> AwaitRestartSafe for LibFuzzerEventManager<I, MT, S> {
    fn await_restart_safe(&mut self) {}
}

impl<I, MT, S> EventRestarter<S> for LibFuzzerEventManager<I, MT, S>
where
    S: HasCurrentStageId,
{
...
impl<I, MT, S> EventReceiver<I, S> for LibFuzzerEventManager<I, MT, S>
where
    I: Debug,
    MT: Monitor,
    S: Stoppable,
...
impl<I, MT, S> ProgressReporter<S> for LibFuzzerEventManager<I, MT, S>
where
    I: Debug,
    MT: Monitor,
...
impl<I, MT, S> HasEventManagerId for LibFuzzerEventManager<I, MT, S> {
    fn mgr_id(&self) -> EventManagerId {
        EventManagerId(0)
    }
}

impl<I, MT, S> LibFuzzerEventManager<I, MT, S>
where
    I: Debug,
    MT: Monitor,
    S: Stoppable,
{
    /// Creates a new [`LibFuzzerEventManager`].
    ...
    fn handle_in_broker(
        ...
        match event {
            Event::NewTestcase { corpus_size, .. } => {
                ...
                monitor.display(client_stats_manager, "NEW   ", ClientId(0))?;
                Ok(BrokerEventResult::Handled)
            }
            Event::Heartbeat => {
                monitor.display(client_stats_manager, "pulse ", ClientId(0))?;
                Ok(BrokerEventResult::Handled)
            }
            ...
            Event::Stop => {
                monitor.display(client_stats_manager, "DONE  ", ClientId(0))?;
...
#[derive(Debug)]
pub struct LibFuzzerRestartingEventManager<I, MT, S, SHM, SP> {
    /// The actual simple event mgr
    inner: LibFuzzerEventManager<I, MT, S>,
    ...
    SP: ShMemProvider<ShMem = SHM>,
{
    ...
    fn launched(monitor: MT, staterestorer: StateRestorer<SHM, SP>) -> Self {
        Self {
            staterestorer,
            inner: LibFuzzerEventManager::new(monitor),
        }
    }
        ...
        MT: Debug,
    {
        ...
        let (state, mgr) = match staterestorer
            .restore::<(S, Duration, HashMap<ClientId, ClientStats>)>()?
        {
            None => {
                ...
                (
                    None,
                    LibFuzzerRestartingEventManager::launched(monitor, staterestorer),
                )
            }
            ...
            Some((state, start_time, clients_stats)) => {
                ...
                let mut this = LibFuzzerRestartingEventManager::launched(monitor, staterestorer);
