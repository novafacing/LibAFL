    ...
    pub fn on_port(shmem_provider: SP, port: u16) -> Result<Self, Error> {
        match tcp_bind(port) {
            Ok(listener) => {
                ...
                let mut broker = LlmpBroker::builder().shmem_provider(shmem_provider).build();
                broker.launch_listener(Listener::Tcp(listener))?;
...
/// 
/// The `LlmpReceiver` can be inialized from:
///
/// - An environment variable like `_XXX_RECEIVER`
/// - An existing shared memory map
/// - An `LlmpDescription`
///
/// # Examples
/// 
/// ## New sender with ID keeping all pages (i.e. for a broker)
/// 
/// ```rust,ignore
/// LlmpSender::builder()
///     .id(id /* An existing id */)
///     .shmem_provider(shmem_provider /* An existing provider */)
///     .keep_pages_forever(true)
///     .build()
/// ```
/// 
/// ## New sender, not keeping pages (i.e. for a client)
/// 
/// ```rust,ignore
/// LlmpSender::builder()
///     .id(id /* An existing id */)
...
/// ```
/// 
/// ## New sender from environment variable
/// 
/// ```rust,ignore
/// LlmpSender::builder()
///     .shmem_provider(shmem_provider /* An existing provider */)
///     .source_env("_ENV_VAR_SENDER")
///     .build()
/// ```
/// 
/// ## New sender from description
/// 
/// ```rust,ignore
/// LlmpSender::builder()
///     .shmem_provider(shmem_provider /* An existing provider */)
///     .source_description(source_description /* An existing description */)
///     .build()
/// ```
/// 
/// ## New sender from existing shared memory
/// 
/// ```rust,ignore
/// LlmpSender::builder()
///     .shmem_provider(shmem_provider /* An existing provider */)
///     .source_shmem(source_shmem /* An existing shared memory */)
///     .build()
/// ```
    ...
    SP: ShMemProvider,
{
    ...
    #[allow(unused)]
    #[builder(default, setter(strip_option))]
    source_description: Option<LlmpDescription>,
    #[allow(unused)]
    #[builder(default, mutable_during_default_resolution, setter(strip_option))]
    source_shmem: Option<SP::ShMem>,
    #[allow(unused)]
    #[builder(default)]
    source_last_msg_sent_offset: Option<u64>,


    #[builder(
        default = {
            vec![
                if let Some(ref source_env) = source_env {
                    let map = LlmpSharedMap::existing(
                        shmem_provider.existing_from_env(source_env)
                            .unwrap_or_else(|e| {
                                panic!("No existing shared memory found for \
                                    environment variable {source_env}: {e}");
                            })
                    );

                    if let Some(source_env_id) = LlmpSender::<SP>::client_id_from_env(source_env)
                        .unwrap_or_else(|e| {
                            panic!("No client ID for environment variable {source_env}: {e}");
                        }) {
                            id = source_env_id;
                    }

                    map
                } else if let Some(ref source_description) = source_description {
                    LlmpSharedMap::existing(
                        shmem_provider.shmem_from_description(source_description.shmem)
                            .unwrap_or_else(|e| {
                                panic!("No existing shared memory found for \
                                    description {source_description:?}: {e}");
                            })
                    )
                } else if let Some(source_shmem) = source_shmem.take() {
                    let map = LlmpSharedMap::existing(source_shmem);
                    id = unsafe { (*map.page()).sender_id };
                    map
                } else {
                    LlmpSharedMap::new(
                        id,
                        shmem_provider.new_shmem(LLMP_CFG_INITIAL_MAP_SIZE)
                            .unwrap_or_else(|e| {
                                panic!("Unable to create new shmem with size \
                                    {LLMP_CFG_INITIAL_MAP_SIZE}: {e}");
                            })
                    )
                }
            ]
    ...
    #[builder(
        default = {
            if let Some(ref source_env) = source_env {
                if let Some(offset) = msg_offset_from_env(source_env)
                    .unwrap_or_else(|e| {
                        panic!("No message offset for source environment variable \
                            {source_env}: {e}");
                    }) {
                    out_shmems.first_mut()
                        .unwrap_or_else(|| {
                            panic!("No out shmem set");
                        })
                        .msg_from_offset(
                            offset
                        )
                        .unwrap_or_else(|e| {
                            panic!("No message for offset {offset}: {e}");
                        })
                } else {
                    ptr::null()
                }
            } else if let Some(ref source_description) = source_description {
                if let Some(offset) = source_description.last_message_offset {
                    out_shmems.first_mut()
                        .unwrap_or_else(|| {
                            panic!("No out shmem set");
                        })
                ...
                source_last_msg_sent_offset
                    .map_or(ptr::null(), |o| {
                        out_shmems.first_mut()
                            ...
                            .msg_from_offset(o)
                            .unwrap_or_else(|e| {
                                panic!("No message found for offset {o}: {e}");
                            })
                    })
            }
        }
...
///
/// ```rust,ignore
/// LlmpReceiver::builder()
///     .shmem_provider(shmem_provider /* An existing provider */)
///     .source_env("_ENV_VAR_RECEIVER")
///     .build()
/// ```
///
/// ## From an existing shared memory map
///
/// ```rust,ignore
/// LlmpReceiver::builder()
///     .shmem_provider(shmem_provider /* An existing provider */)
///     .source_shmem(current_broker_shmem /* An existing shmem */)
///     .build()
/// ```
///
...
/// LlmpReceiver::builder()
///     .shmem_provider(shmem_provider /* An existing provider */)
///     .source_description(existing_description /* A description */)
///     .build()
/// ```
///
/// ## From nothing
...
/// LlmpReceiver::builder()
///     .shmem_provider(shmem_provider /* An existing provider */)
///     .current_recv_shmem(recv_shmem /* An existing shared memory */)
///     .build()
/// ```
...
{
    #[builder(mutable_during_default_resolution)]
    // NOTE: Must be set first, because some setting methods utilize the provider during
    // construction
    /// The shmem provider
    ///
    /// If set explicitly, the provided `ShMemProvider` will be used.
    shmem_provider: SP,

    // Temporary fields settable during initialization to control how the `LlmpReceiver` instance
    // is built:
    #[allow(unused)]
    #[builder(default, setter(strip_option))]
    #[cfg(feature = "std")]
    // NOTE: Equivalent of `on_existing_from_env`
    source_env: Option<String>,

    #[allow(unused)]
    #[builder(default, setter(strip_option))]
    // NOTE: Equivalent of `on_existing_from_description`
    source_description: Option<LlmpDescription>,

    #[allow(unused)]
    #[builder(default, mutable_during_default_resolution, setter(strip_option))]
    // NOTE: Equivalent of `on_existing_shmem`
    source_shmem: Option<SP::ShMem>,
    ...
    #[builder(
        default = {
            if let Some(ref source_env) = source_env {
                msg_offset_from_env(source_env)
                    .unwrap_or_else(|e| {
                        panic!("No message offset found for environment variable {source_env}: {e}")
                    })
            } else if let Some(ref source_description) = source_description {
                source_description.last_message_offset
            } else {
                Default::default()
            }
        },
    )]
    // NOTE: Used in on_existing_shmem, set different ways depending on which build strategy
    //       is being used
    source_last_msg_recvd_offset: Option<u64>,

    #[builder(
        default = {
            LlmpSharedMap::existing(if let Some(ref source_env) = source_env {
                shmem_provider.existing_from_env(source_env)
                    .unwrap_or_else(|e| {
                        panic!("No shared map found for environment variable {source_env}: {e}")
                    })
            } else if let Some(ref source_description) = source_description {
                shmem_provider.shmem_from_description(source_description.shmem)
                    .unwrap_or_else(|e| {
                        panic!("No shared map found for description {source_description:?}: {e}")
                    })
            } else {
                source_shmem.take()
                    .unwrap_or_else(|| {
                        panic!("No source shared memory set and no source \
                            environment variable or description set")
                    })
            })
        }, 
        mutable_during_default_resolution
    )]
    // NOTE: Used in on_existing_shmem (and by extension, from_description and from_env)
    ...
    #[builder(
        default = {
            if let Some(ref source_last_msg_recvd_offset) = source_last_msg_recvd_offset {
                current_recv_shmem.msg_from_offset(*source_last_msg_recvd_offset)
                    .unwrap_or_else(|e| panic!("No message found for offset {source_last_msg_recvd_offset}: {e}"))
            } else {
                ptr::null()
            }
        }
    ...
    SHM: ShMem,
{
    ...
    #[cfg(feature = "std")]
    pub fn msg_from_env(&mut self, map_env_name: &str) -> Result<*const LlmpMsg, Error> {
    ...
    #[allow(clippy::cast_ptr_alignment)]
    pub fn msg_from_offset(&mut self, offset: u64) -> Result<*const LlmpMsg, Error> {
        ...
        if offset > page_size {
            ...
            unsafe { Ok(((*page).messages.as_mut_ptr() as *mut u8).add(offset) as *const LlmpMsg) }
        }
    }
}
...
#[cfg(feature = "std")]
#[derive(Debug)]
/// Wrapper for the id and handle of a listener for an `LlmpBroker`
pub struct LlmpBrokerListener {
    id: ClientId,
    #[cfg(feature = "std")]
...
#[cfg(feature = "std")]
impl LlmpBrokerListener {
    fn join(&mut self) {
        #[cfg(unix)]
        {
            if self.handle.as_ref().is_some_and(thread::JoinHandle::is_finished) {
                if let Some(h) = self.handle.take() {
                    h.join().expect("Failed to join listener thread");
                }
            } else if let Some(h) = self.handle.take() {
                unsafe { pthread_cancel(h.into_pthread_t()) };
            }
        }
        #[cfg(windows)]
        {
            // On windows, we hope for the best
            if let Some(h) = self.handle.take() {
                h.join().expect("Failed to join listener thread");
            }
...
/// - Just a shmem provider
/// - a shmem provider and whether to keep pages
/// - a provider, port, and whether to keep pages
    ...
    SP: ShMemProvider + 'static,
{
    ...
    #[builder(
        default = LlmpSender::builder()
            .shmem_provider(shmem_provider.clone())
            .out_shmems(vec![
                LlmpSharedMap::new(ClientId(0), shmem_provider.new_shmem(next_shmem_size(0)).expect("Failed to create new shared memory"))
            ])
    ...
    num_clients_total: u32,
    #[cfg(feature = "std")]
    #[builder(
        default = ports.iter().map(|p| {
            let llmp_out = &llmp_out;
            let llmp_tcp_id = ClientId(num_clients_total);

            let mut tcp_out_shmem = LlmpSharedMap::new(
                llmp_tcp_id,
                shmem_provider.new_shmem(LLMP_CFG_INITIAL_MAP_SIZE).expect("Failed to create new shared map")
            );
            let tcp_out_shmem_description = tcp_out_shmem.shmem.description();

            {
                // Inline the register_client/add_client process
                tcp_out_shmem.mark_safe_to_unmap();
                let mut llmp_client = LlmpReceiver::builder()
                    .current_recv_shmem(tcp_out_shmem)
                    .shmem_provider(shmem_provider.clone())
                    .build();
                
                llmp_client.id = llmp_tcp_id;
                llmp_clients.push(llmp_client);
                num_clients_total += 1;
            }

            let handle = LlmpBroker::launch_listener_impl(
                llmp_out,
                llmp_tcp_id,
                tcp_out_shmem_description,
                Listener::Tcp(tcp_bind(*p).expect("Failed to bind on port")),
            );

            LlmpBrokerListener { id: llmp_tcp_id, handle: Some(handle) }
        })
        .collect(),
        setter(skip)
    )]
    ...
    listeners: Vec<LlmpBrokerListener>,
    #[cfg(not(feature = "std"))]
    #[builder(default)]
    ...
    #[builder(default = CLIENT_TIMEOUT)]
    ...
    SP: ShMemProvider + 'static,
{
    ...
    pub fn peek_next_client_id(&self) -> ClientId {
        ClientId(self.num_clients_total)
    }
        ...
        for i in 0..self.llmp_clients.len() {
            ...
            match unsafe { self.handle_new_msgs(client_id, on_new_msg) } {
                Ok(has_messages) => {
                    ...
                    #[cfg(feature = "std")]
                    if !has_messages && !self.listeners.iter().any(|x| x.id == client_id) {
        ...
        F: FnMut(Option<(ClientId, Tag, Flags, &[u8])>) -> Result<LlmpMsgHookResult, Error>,
    {
        ...
        while !self.is_shutting_down() {
            ...
            if let Some(exit_after_count) = self.exit_cleanly_after {
                ...
                if !self.has_clients()
                    && (self.num_clients_total as usize - self.listeners.len()) >= exit_after_count.into()
                {
    ...
    #[cfg(feature = "std")]
    pub fn launch_tcp_listener_on(&mut self, port: u16) -> Result<(), Error> {
    ...
    fn b2b_thread_on(
        ...
        thread::spawn(move || {
            ...
            let mut local_receiver = LlmpReceiver::builder()
                .shmem_provider(shmem_provider_bg)
                .source_description(LlmpDescription {
    ...
    fn launch_listener_impl(
        llmp_out: &LlmpSender<SP>,
        llmp_tcp_id: ClientId,
        tcp_out_shmem_description: ShMemDescription,
        listener: Listener,
    ) -> thread::JoinHandle<()>
    where
        SP: ShMemProvider,
    {
        ...
        let client_out_shmem_mem = &llmp_out.out_shmems.first().expect("No output shared memories in llmp sender").shmem;
        ...
        thread::spawn(move || {
            ...
            let mut shmem_provider_bg = SP::new().expect("Unable to create a new shared memory provider in listener thread");

            let mut current_client_id = ClientId(llmp_tcp_id.0 + 1);

            let mut tcp_incoming_sender = LlmpSender::builder()
                .id(llmp_tcp_id)
                .out_shmems(vec![LlmpSharedMap::existing(
                    shmem_provider_bg
                        .shmem_from_description(tcp_out_shmem_description)
                        .expect("Failed to get shared memory from description"),
                )])
                .shmem_provider(shmem_provider_bg.clone())
                .build();
                ...
                match listener.accept() {
                    ListenerStream::Tcp(mut stream, addr) => {
                        log::info!(
                            "New connection: {:?}/{:?}",
                            addr,
                            stream.peer_addr().expect("Unable to get peer address of TCP stream")
                        );
    ...
    pub fn launch_listener(&mut self, listener: Listener) -> Result<(), Error> {
        ...
        let llmp_out = &self.llmp_out;

        assert_eq!(llmp_tcp_id, listener_id, "LLMP TCP ID and Listener ID did not match");

        let handle = Self::launch_listener_impl(
            llmp_out,
            llmp_tcp_id,
            tcp_out_shmem_description,
            listener,
        ...
        self.listeners.push(LlmpBrokerListener {
            id: listener_id,
            handle: Some(handle),
        });

        // NOTE: We no longer return the handle here because it was typically unused, and you
    ...
    SP: ShMemProvider,
{
    ...
    #[allow(unused)]
    #[builder(default, setter(strip_option))]
    source_description: Option<LlmpClientDescription>,
    #[allow(unused, clippy::option_option)]
    #[builder(default)]
    source_last_msg_recvd_offset: Option<Option<u64>>,
    #[allow(unused)]
    #[builder(default, setter(strip_option))]
    port: Option<u16>,

    #[allow(unused, unused_mut, clippy::type_complexity)]
    #[builder(
        default = {
            if let Some(port) = port {
                ...
                let TcpResponse::BrokerConnectHello {
                    ...
                    hostname: _,
                        ...
                        .unwrap_or_else(|e| panic!("Unable to receive TCP message from stream: {e}"))
                        .try_into()
                        .unwrap_or_else(|e| panic!("TCP response was an incorrect type: {e}"))
                else {
                   panic!("Received unexpected Broker Hello");
                };

                let map = LlmpSharedMap::existing(
                    shmem_provider.shmem_from_description(broker_shmem_description)
                        .unwrap_or_else(|e| {
                            panic!("Unable to get shared memory from description: {e}")
                        })
                );

                let mut sender = LlmpSender::builder()
                    .shmem_provider(shmem_provider.clone())
                    .build();

                let receiver = LlmpReceiver::builder()
                    .shmem_provider(shmem_provider.clone())
                    .current_recv_shmem(map)
                    .build();

                let client_hello_req = TcpRequest::LocalClientHello {
                    shmem_description: sender.out_shmems.first()
                        .unwrap_or_else(|| panic!("No shared memory in sender"))
                        .shmem
                        .description()
                ...
                send_tcp_msg(&mut stream, &client_hello_req)
                    .unwrap_or_else(|e| panic!("Failed to send local client hello: {e}"));

                let TcpResponse::LocalClientAccepted { client_id } = recv_tcp_msg(&mut stream)
                    .unwrap_or_else(|e| panic!("Failed to receive response: {e}"))
                    .try_into()
                    .unwrap_or_else(|e| panic!("Failed to convert response: {e}")) else {
                        panic!("Failed");
                    };
                ...
                unsafe { 
                    (
                        *sender.out_shmems.first_mut()
                            .unwrap_or_else(|| {
                                panic!("No shared memory in sender")
                            })
                            .page_mut()
                    )
                    .sender_id = client_id; 
                ...
                Some((Some(sender), Some(receiver)))
            } else {
                None
            }
        },
    ...
    connected: Option<(Option<LlmpSender<SP>>, Option<LlmpReceiver<SP>>)>,


    #[builder(
        default = {
            // port: source_shmem_provider, port
            // env: source_shmem_provider, source_env
            // shmem: source_shmem_provider, source_shmem, source_last_msg_recvd_offset
            // default: source_shmem_provider, source_shmem, source_id
            // description: source_shmem_provider, source_description
            // p2p: source_shmem_provider, source_id
            if let Some(ref mut connected) = connected {
                connected.0.take().unwrap_or_else(|| panic!("No connected sender"))
            } else if let Some(ref source_env) = source_env {
                LlmpSender::builder()
                    .shmem_provider(shmem_provider.clone())
                    .source_env(format!("{source_env}_SENDER"))
                    .build()
            } else if let Some(ref source_shmem) = source_shmem {
                if let Some(ref source_last_msg_recvd_offset) = source_last_msg_recvd_offset {
                    LlmpSender::builder()
                        .shmem_provider(shmem_provider.clone())
                        .source_shmem(source_shmem.clone())
                        .source_last_msg_sent_offset(*source_last_msg_recvd_offset)
                        .build()
                } else if let Some(ref source_id) = source_id {
                    LlmpSender::builder()
                        .shmem_provider(shmem_provider.clone())
                        .id(*source_id)
                        .source_shmem(source_shmem.clone())
                    ...
                    panic!("Source shared memory provided but no last offset or id provided");
                }
            } else if let Some(ref source_description) = source_description {
                LlmpSender::builder()
                    .shmem_provider(shmem_provider.clone())
                    .source_description(source_description.sender)
                    .build()
            } else if let Some(ref source_id) = source_id {
                LlmpSender::builder()
                    .shmem_provider(shmem_provider.clone())
    ...
    #[builder(
        default = {
            ...
            if let Some(ref mut connected) = connected {
                connected.1.take().unwrap_or_else(|| panic!("No connected receiver"))
            } else if let Some(ref source_env) = source_env {
                    LlmpReceiver::builder()
                        .shmem_provider(shmem_provider.clone())
                        .source_env(format!("{source_env}_RECEIVER"))
                        .build()
            } else if let Some(ref source_shmem) = source_shmem {
                if let Some(ref source_last_msg_recvd_offset) = source_last_msg_recvd_offset {
                    LlmpReceiver::builder()
                        .shmem_provider(shmem_provider.clone())
                        .source_shmem(source_shmem.clone())
                        .source_last_msg_recvd_offset(*source_last_msg_recvd_offset)
                        .build()
                } else if let  Some(ref source_id) = source_id {
                    LlmpReceiver::builder()
                        .shmem_provider(shmem_provider.clone())
                ...
                LlmpReceiver::builder()
                    .shmem_provider(shmem_provider.clone())
                    .source_description(source_description.receiver)
                    .build()
            } else if let Some(ref source_id) = source_id {
                LlmpReceiver::builder()
                    .shmem_provider(shmem_provider.clone())
...
/// 
/// Clients can be initialized from:
/// 
/// - An environment variable
/// - A shmem description
/// - An existing shared memory
...
mod tests {
    ...
    pub fn test_llmp_connection() {
        ...
        client = LlmpClient::builder()
            .shmem_provider(shmem_provider)
            .source_env("_ENV_TEST".to_string())
            .build();
